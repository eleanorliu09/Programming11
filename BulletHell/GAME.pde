...
void addObjects() {
  ...
  //if (frameCount % 600 == 0) luckyObject();
}
...
void luckyObject() {
  int luckyNum = 1 + (int) random(0, 2);

  if (luckyNum == 1) obj.add(new Heart());
  else if (luckyNum == 2) obj.add(new Potion());
  else if (luckyNum == 3) obj.add(new Lightning());
}
